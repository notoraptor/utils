...
def theano_spatialtf_from_grid(inp, grid):
    ...
    return theano_spatialtf_bilinear_kernel(T.dot(all_x_neg_ones, M1))
...
inp_val = np.random.random(inp_shape).astype(dtype)
theta_val = np.asarray(inp_shape[0] * [transform], dtype=dtype)
# theta_val += 1e-5
...
print('Verifying ...')
utt.verify_grad(grad_theta_functor, [theta_val], n_tests=10)
print('Ok.')
